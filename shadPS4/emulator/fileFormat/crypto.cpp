...
void Crypto::decryptPFS(CryptoPP::byte* dataKey, CryptoPP::byte* tweakKey, U08* src_image, CryptoPP::byte*& dst_image, int length, U64 sector) {
	for (int i = 0; i < length; i += 0x1000) {// start at 0x10000 to keep the header when decrypting the whole pfs_image.
