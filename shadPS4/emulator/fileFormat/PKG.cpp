	...
	for (int i = 0; i < fs_table.size(); i++) {
		...
		if (inode_type == PFS_CURRENT_DIR) {
			...
			for (int j = 0; j < nblocks; j++) {
				...
				U64 sectorOffset = sectorMap[sector_loc + j]; // offset into PFSC_image and not pfs_image.
				U64 sectorSize = sectorMap[sector_loc + j + 1] - sectorOffset; // indicates if data is compressed or not.
				U64 fileOffset = (pkgheader.pfs_image_offset + pfsc_pos + sectorOffset);
				U64 currentSector1 = (pfsc_pos + sectorOffset) / 0x1000; // block size is 0x1000 for xts decryption.
				...
				file.Seek(fileOffset - previousData, fsSeekSet);
				file.ReadBytes(pfsc, pfsc_buf_size);

				PKG::crypto.decryptPFS(crypto_keys.dataKey, crypto_keys.tweakKey, pfsc, pfs_decrypted, pfsc_buf_size, currentSector1);
				...
				if (j < nblocks - 1)
					...
					inflated.Write(decompressedData, 0x10000 - (size_decompressed - bsize)); // This is to remove the zeros at the end of the file.
